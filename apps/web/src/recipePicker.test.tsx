...
// Use a fixed South-Indian-dominant subset so assertions don't depend on the full library's mix.
const SI = ["rec_chutney", "rec_rasam", "rec_rasam_simple", "rec_curdrice", "rec_lemonrice", "rec_beetroot", "rec_sambar", "rec_sambar_simple", "rec_sambar_complex"];
const ITALIAN = ["rec_aglio", "rec_tompasta"];
const LIB = goldenLibrary.filter((r) => [...SI, ...ITALIAN].includes(r.recipeId));

describe("RecipePicker", () => {
  it("default-opens the largest cuisine and adds a dish on tap", () => {
    const onPick = vi.fn();
    render(<RecipePicker library={LIB} onPick={onPick} />);
  ...
  it("collapses and expands a cuisine accordion", () => {
    render(<RecipePicker library={LIB} onPick={() => {}} />);
  ...
  it("shows Recently cooked / You cook these often from history", () => {
    ...
    render(<RecipePicker library={LIB} notes={notes} onPick={() => {}} />);
