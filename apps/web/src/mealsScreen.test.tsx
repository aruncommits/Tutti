...
describe("MealsScreen (Brief v12 item 5)", () => {
  it("lists a saved meal and restores it on tap", () => {
    ...
    render(<MealsScreen meals={[meal]} recipes={thaliV1.recipes} onRestore={onRestore} onRemove={() => {}} />);
  ...
  it("removes a meal via the × control", () => {
    ...
    render(<MealsScreen meals={[meal]} recipes={thaliV1.recipes} onRestore={() => {}} onRemove={onRemove} />);
    fireEvent.click(screen.getByRole("button", { name: /remove sunday thali/i }));
    expect(onRemove).toHaveBeenCalledWith("m1");
  });

  it("shows an empty state when there are no meals", () => {
    render(<MealsScreen meals={[]} recipes={thaliV1.recipes} onRestore={() => {}} onRemove={() => {}} />);
